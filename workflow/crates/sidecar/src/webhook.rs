...
use std::sync::Arc;
use workflow_types::{
    parse_deps, parse_priority, parse_timeout, Job, JobState, WebhookAction, WebhookPayload,
};
...
pub async fn handle_webhook(state: &Arc<AppState>, payload: WebhookPayload) -> Result<()> {
    ...
    match payload.action {
        WebhookAction::Opened => handle_opened(state, owner, repo, issue).await?,
        WebhookAction::Edited => handle_edited(state, owner, repo, issue).await?,
        WebhookAction::Closed => handle_closed(state, owner, repo, issue).await?,
    ...
    if let Ok(payload_bytes) = serde_json::to_vec(&payload.issue).map(bytes::Bytes::from) {
...
async fn handle_opened(
    ...
    if !rejected_deps.is_empty() {
        ...
        let _ = state
            .forgejo
            .post_comment(owner, repo, issue.number, &msg)
            .await;
    }

    // Determine correct state unless explicitly held
    if !on_ice {
        let resolved_state = if !dep_numbers.is_empty() && !state.graph.all_deps_done(&job_key)? {
            JobState::Blocked
        } else {
            JobState::OnDeck
        };
        ...
        if forgejo_state.as_ref() != Some(&job.state) {
            state
                .forgejo
                .set_job_state(owner, repo, issue.number, &job.state)
                .await?;
        }
    }
...
async fn handle_edited(
    ...
    if should_re_evaluate && !issue.labels.iter().any(|l| l.name == "status:on-ice") {
        let new_state = if !dep_numbers.is_empty() && !state.graph.all_deps_done(&job_key)? {
            JobState::Blocked
        } else {
            JobState::OnDeck
        };
    ...
    if on_ice {
        ...
        if let Some(job) = current {
            if job.state == JobState::OnIce {
                ...
                let new_state = if !dep_keys.is_empty() && !state.graph.all_deps_done(&job_key)? {
                    JobState::Blocked
                } else {
                    JobState::OnDeck
                };
